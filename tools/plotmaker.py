...
def fitmaker(post,params,parameters,inj,Model,Injection=None,saveto=None,plot_convolved=True,astro_kwargs=None,det_kwargs=None):
...
#    fs = frange[ffilt][::10]
    ...
    plt.ylabel('PSD [1/Hz]')
    if saveto is not None:
        plt.savefig(saveto + '/spectral_fit_astro.png', dpi=150)
    else:
    ...
    if plot_convolved:
        ...
        plt.ylabel('PSD [1/Hz]')
        if saveto is not None:
            plt.savefig(saveto + '/spectral_fit_detector.png', dpi=150)
        else:
