...
def plotmaker(post, params,parameters, inj, Model, param_type='all', Injection=None,saveto=None):
    ...
    plotted_parameters = Model.parameters[param_type]
    filter_params = [(p in plotted_parameters) for p in Model.parameters['all']]
    ...
    if not params['load_data']:
        ...
        truevals = {param:inj_truevals[param] for param in plotted_parameters if param in inj_truevals.keys()}
    ...
    npar = len(plotted_parameters) # npar = Model.Npar
    ...
    cc = ChainConsumer()
    cc.add_chain(post[:,filter_params], parameters=plotted_parameters)
    ...
    for ii in range(npar):
        ...
        sum_ax = sum_data[plotted_parameters[ii]]
        ...
        label =  plotted_parameters[ii][:-1] + ' = ' + mean_form + '^{+' + err[0] + '}_{-' + err[1] + '}'+exp_form+'$'
    ...
    if param_type == 'all':
        ...
        print("Posterior plots printed in " + params['out_dir'] + "corners.png")
    else:
        ## Save posterior with added key in name
        if saveto is not None:
            plt.savefig(saveto + param_type + '_corners.png', dpi==200)
        else:
            plt.savefig(params['out_dir'] + param_type + '_corners.png')
        print("Posterior plots for " + param_type + " parameters printed in " + params['out_dir'] + param_type + "_corners.png")
        plt.close()
