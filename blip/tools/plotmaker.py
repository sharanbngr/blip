...
def mapmaker(post, params, parameters, Model, saveto=None, coord=None, cmap=None, post_map_kwargs={}, med_map_kwargs={}, plot_data_path=None):
    ...
    saveto (str) : /path/to/save/skymaps/ (Defaults to params['out_dir']).
    ...
    plot_data_path (str) : /path/to/plot_data.pickle; where to save the plot data as a pickle file.
                             ...
                             Defaults to params['out_dir']/plot_data.pickle
    ...
    print("Spatial fit maps saved to {}".format(plot_data_path))
...
def fitmaker(post,params,parameters,inj,Model,Injection=None,saveto=None,plot_convolved=True,astro_kwargs={},det_kwargs={}, plot_data_path=None):
    ...
    *_kwargs : dict
    ...
    ## store info
    plot_data['fit_data']['fs'] = fs.flatten()
    plot_data['fit_data']['fs_full'] = frange
    plot_data['fit_data']['fmin'] = params['fmin']
    plot_data['fit_data']['fmax'] = params['fmax']
    plot_data['fit_data']['astro_fits'] = {}
    plot_data['fit_data']['det_fits'] = {}
    plot_data['fit_data']['astro_inj'] = {}
    plot_data['fit_data']['det_inj'] = {}
    ...
    ## loop over submodels
    signal_model_names = [sm_name for sm_name in Model.submodel_names if sm_name not in ['noise','fixednoise']] 
    ...
    if len(signal_model_names) > 0:
        ...
        for i, sm_name in enumerate(signal_model_names):
            ...
            plot_data['fit_data']['astro_fits'][sm_name] = {}
            ...
            if hasattr(sm,"fixedspec") and sm.fixedspec:
                ...
                plot_data['fit_data']['astro_fits'][sm_name]['median'] = Sgw_median
                plot_data['fit_data']['astro_fits'][sm_name]['upper95'] = Sgw_upper95
                plot_data['fit_data']['astro_fits'][sm_name]['lower95'] = Sgw_lower95
        ...
        if not params['load_data']:
            ...
            for component_name in Injection.component_names:
                if component_name != 'noise':
                    plot_data['fit_data']['astro_inj'][component_name] = {}
                    ...
                    plot_data['fit_data']['astro_inj'][component_name]['spec'] = inj_Sgw_filt_i
                    plot_data['fit_data']['astro_inj'][component_name]['spec_full'] = inj_Sgw_i
...
#                    ywmeds.append(weighted_ymed)
    ...
    if plot_convolved:
        ...
        for sm_name in Model.submodel_names:
            ...
            plot_data['fit_data']['det_fits'][sm_name] = {}
...
#                    Sgw_j = np.mean(sm.compute_Sgw(fdata,post_sm)[:,None] * sm.compute_summed_response(sm.compute_skymap_alms(post_sm_sph))[0,0,filt,:],axis=1)
            ...
            start_idx += sm.Npar
            if hasattr(sm,"fixedspec") and sm.fixedspec and not hasattr(sm,"spatial_start") and not hasattr(sm,"spatial_start"):
                Sgw_fixed = Sgw[0,:]
                plt.loglog(fdata,Sgw_fixed,color=sm.color)
            else:
                ## get summary statistics
                ## median and 95% C.I.
                ...
                plot_data['fit_data']['det_fits'][sm_name]['median'] = Sgw_median
                plot_data['fit_data']['det_fits'][sm_name]['upper95'] = Sgw_upper95
                plot_data['fit_data']['det_fits'][sm_name]['lower95'] = Sgw_lower95
                for Sgw_quantile in [Sgw_median,Sgw_lower95]:
                    log_Sgw_i = np.log10(Sgw_quantile[np.nonzero(Sgw_quantile)])
                    ymins.append(np.min(log_Sgw_i))
                    ymeds.append(np.median(log_Sgw_i))
        ...
        if not params['load_data']:
            ...
            for component_name in Injection.component_names:
                ...
                plot_data['fit_data']['det_inj'][component_name] = {}
                ...
                if component_name == 'noise':
                    plot_data['fit_data']['det_inj'][component_name]['spec'] = Injection.plot_injected_spectra(component_name,fs_new=fdata,return_PSD=True,channels='22',ymins=ymins,**kwargs)
                else:
                    plot_data['fit_data']['det_inj'][component_name]['spec'] = Injection.plot_injected_spectra(component_name,fs_new='data',return_PSD=True,convolved=True,ymins=ymins,**kwargs)
                    if component_name not in Model.submodel_names and component_name not in signal_aliases:
                        model_legend_elements.append(Line2D([0],[0],color=Injection.components[component_name].color,lw=3,label=Injection.components[component_name].fancyname))
    ...
    if os.path.exists(plot_data_path):
        ...
        with open(plot_data_path, 'wb') as datafile:
            plot_data = pickle.dump(plot_data,datafile)
    print("Spectral fit curves saved to {}".format(plot_data_path))
...
if __name__ == '__main__':
    ...
    if not args.nomap:
        if 'healpy_proj' in params.keys():
            ...
            mapmaker(post, params, parameters, Model, cmap=params['colormap'], plot_data_path=args.plotdatadir)
