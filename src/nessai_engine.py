...
class nessai_engine():
    ...
    @classmethod
    def define_engine(cls, lisaobj, params, nlive, nthread, seed, output, pool=None, resume=False):
        ...
        if params['modeltype']=='isgwb':
            ...
            if params['spectrum_model']=='powerlaw':
                ...
                engine = FlowSampler(model,nlive=nlive,output=output,seed=seed,stopping=0.1,pool=pool,n_pool=pool_size)
#                
...
#            raise TypeError("Checkpoint file <{}> does not exist. Cannot resume from checkpoint.".format(resume_file))
    ...
    def load_engine(params,nlive,nthread,seed,output,pool):
    ...
    @staticmethod
    def run_engine_with_checkpointing(engine,parameters,model,output,interval,blip_checkpoint_file):
        ...
        unit_samples = [np.array(res['posterior_samples'][name]) for name in parameters]
