...
class orbitinglisa():
    ...
    def isgwb_omich_response(self, f0, tsegmid):
        ...
        ## udir is just u-hat.omega, where u-hat is the u unit vector and omega is the unit vector in the sky direction of the GW signal
        #udir = ( np.tensordot((x2-x1)/Lu,np.cos(phi)*np.sqrt(1 - ctheta**2) )+ ((y2-y1)/Lu)*np.sin(phi)*np.sqrt(1 - ctheta**2) + (((z2-z1))/Lu)*ctheta
        #vdir = ((x3-x1)/Lu)*np.cos(phi)*np.sqrt(1 - ctheta**2) + ((y3-y1)/Lu)*np.sin(phi)*np.sqrt(1 - ctheta**2) + (((z3-z1))/Lu)*ctheta
        #wdir = ((x3-x2)/Lu)*np.cos(phi)*np.sqrt(1 - ctheta**2) + ((y3-y2)/Lu)*np.sin(phi)*np.sqrt(1 - ctheta**2) + (((z3-z2))/Lu)*ctheta
