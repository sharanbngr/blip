...
def Antennapatterns():
    ...
    for ii in range(0, f0.size):
        ...
        if chnl_type=="Doppler":

            # There are for the doppler tracking channels
            R1[ii] = dct*dphi/(4*np.pi)*np.sum((np.absolute(Fplus_u))**2 + (np.absolute(Fcross_u))**2)
            R2[ii] = dct*dphi/(4*np.pi)*np.sum((np.absolute(Fplus_v))**2 + (np.absolute(Fcross_v))**2)
            R3[ii] = dct*dphi/(4*np.pi)*np.sum((np.absolute(Fplus_w))**2 + (np.absolute(Fcross_w))**2)



        elif chnl_type=="Michelson":
            ...
            R1[ii] = dct*dphi/(4*np.pi)*np.sum((np.absolute(Fplus1))**2 + (np.absolute(Fcross1))**2)
            R2[ii] = dct*dphi/(4*np.pi)*np.sum((np.absolute(Fplus2))**2 + (np.absolute(Fcross2))**2)
            R3[ii] = dct*dphi/(4*np.pi)*np.sum((np.absolute(Fplus3))**2 + (np.absolute(Fcross3))**2)
            ...
            FAplus = (2/3)*np.sin(2*f0[ii])*(2*Fplus1 - Fplus2 - Fplus3)
            FEplus = (2/np.sqrt(3))*np.sin(2*f0[ii])*(Fplus3 - Fplus2)
            FTplus = (1/3)*np.sin(2*f0[ii])*(Fplus1 + Fplus3 + Fplus2)

            FAcross = (2/3)*np.sin(2*f0[ii])*(2*Fcross1 - Fcross2 - Fcross3)
            FEcross = (2/np.sqrt(3))*np.sin(2*f0[ii])*(Fcross3 - Fcross2)
