...
class freqDomain():
    ...
    def isgwb_mich_strain_response(self, f0):
        ...
        for ii in range(0, f0.size):
            ...
            R1[ii, 0], R1[ii, 1] = (2*np.pi/Fplus1.size)*np.sum(Fplus1*rand_plus), dct*dphi*np.sum(Fcross1*rand_cross) 
            R2[ii, 0], R2[ii, 1] = (2*np.pi/Fplus1.size)*np.sum(Fplus2*rand_plus), dct*dphi*np.sum(Fcross2*rand_cross) 
            R3[ii, 0], R3[ii, 1] = (2*np.pi/Fplus1.size)*np.sum(Fplus3*rand_plus), dct*dphi*np.sum(Fcross3*rand_cross) 
